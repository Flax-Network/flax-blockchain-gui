...
from flax.util.beta_metrics import metrics_log_interval_max, metrics_log_interval_min
from flax.util.flax_logging import get_beta_logging_config
from flax.util.errors import InvalidPathError
from flax.util.misc import format_bytes, prompt_yes_no, validate_directory_writable


def default_beta_root_path() -> Path:
    return Path(os.path.expanduser(os.getenv("FLAX_BETA_ROOT", "~/flax-beta-test"))).resolve()
...
def prepare_flax_blockchain_log(path: Path) -> None:
    # TODO: Do stuff we want to do with the logs before submission. Maybe even just fully parse them and
    #  create some final result files and zip them instead of just the logs.
    print(f"  - {path.name}")
