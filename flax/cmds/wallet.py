...
from flax.cmds.cmds_util import execute_with_wallet
...
@wallet_cmd.command("send", short_help="Send flax to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option("-a", "--amount", help="How much flax to send, in XFX", type=str, required=True)
...
@click.option("-t", "--address", help="Address to send the XFX", type=str, required=True)
@click.option(
...
@click.option(
    "-ma",
    "--min_coin_amount",
    help="Ignore coins worth less then this much XFX or CAT units",
    type=str,
    required=False,
    default="0",
)
...
@wallet_cmd.command("make_offer", short_help="Create an offer of XFX/CATs for XFX/CATs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in XFX.",
    type=str,
    default="0",
    show_default=True,
