...
class CATWallet:
    ...
    async def generate_signed_transaction(
        ...
        if flax_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=flax_tx.confirmed_at_height,
                    created_at_time=flax_tx.created_at_time,
                    to_puzzle_hash=flax_tx.to_puzzle_hash,
                    amount=flax_tx.amount,
                    fee_amount=flax_tx.fee_amount,
                    confirmed=flax_tx.confirmed,
                    sent=flax_tx.sent,
                    spend_bundle=None,
                    additions=flax_tx.additions,
                    removals=flax_tx.removals,
                    wallet_id=flax_tx.wallet_id,
                    sent_to=flax_tx.sent_to,
                    trade_id=flax_tx.trade_id,
                    type=flax_tx.type,
                    name=flax_tx.name,
                    memos=[],
                )
