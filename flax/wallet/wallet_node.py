...
class WalletNode:
    ...
    async def is_peer_synced(
        self, peer: WSFlaxConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSFlaxConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSFlaxConnection]:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSFlaxConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            return None
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSFlaxConnection]:
        """
        ...
        synced_and_trusted: List[WSFlaxConnection] = []
        synced: List[WSFlaxConnection] = []
        trusted: List[WSFlaxConnection] = []
        neither: List[WSFlaxConnection] = []
        all_nodes: List[WSFlaxConnection] = self.server.get_full_node_connections().copy()
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peers: List[WSFlaxConnection] = self.get_full_node_peers_in_order()
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSFlaxConnection):
        if self._wallet_state_manager is None:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSFlaxConnection) -> int:
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSFlaxConnection, peak: HeaderBlock
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSFlaxConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def fetch_puzzle_solution(self, height: uint32, coin: Coin, peer: WSFlaxConnection) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSFlaxConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSFlaxConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
