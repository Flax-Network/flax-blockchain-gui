...
This library provides react hooks on the top of @flax-network/api and uses [RTK Query](https://redux-toolkit.js.org/rtk-query/overview) under the hood.
...
- Automatically refresh queries when data changed (using events from Flax Blockchain).
...
import { useGetPublicKeysQuery } from '@flax-network/api-react';
...
import { store, api } from '@flax-network/api-react';
