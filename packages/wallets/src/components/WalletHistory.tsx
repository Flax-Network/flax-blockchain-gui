...
import { Card, CardKeyValue, CopyToClipboard, Flex, Loading, StateColor, TableControlled, toBech32m, useCurrencyCode, mojoToFlaxLocaleString, mojoToCATLocaleString } from '@flax/core';
import { useGetOfferRecordMutation, useGetSyncStatusQuery } from '@flax/api-react';
import styled from 'styled-components';
import type { Row } from '@flax/core';
import { WalletType, TransactionType } from '@flax/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
        ...
        <>
          ...
          <strong>
            {type === WalletType.CAT
              ...
              : mojoToFlaxLocaleString(row.amount)}
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>{mojoToFlaxLocaleString(row.feeAmount)}</strong>
        &nbsp;
        {metadata.feeUnit}
      </>
    ),
