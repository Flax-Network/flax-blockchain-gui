...
import { AlertDialog, Card, Flex, flaxToMojo, mojoToFlaxLocaleString } from '@flax/core';
import {
  send_transaction,
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToFlaxLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = flaxToMojo(amount_input.value);
    const fee = flaxToMojo(fee_input.value);
